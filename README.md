...
Current `main` is the harmonized Phaser/Vite GitHub Pages game produced by PR #78 plus the physical Archive, Two Networks, ClassNet, Referral Vault, Silent Read, Black Vault, and Buckram Gate follow-ups. Every stacked Codex gameplay head and later Perplexity art drop is represented; superseded component PRs are closed by design, not missing. The obsolete five-form DANN-E draft in PR #41 is the sole intentional exclusion because the canonical eight-form roster replaced it. Historical source branches and the deployment branch remain on the remote for provenance and are not pending integrations.

The branch-by-branch evidence and the two restored data payloads are recorded in [`docs/AGENT_PULL_HARMONIZATION.md`](docs/AGENT_PULL_HARMONIZATION.md). Run `npm run audit:pulls` to refresh the remote-branch, PR-head, stacked-ancestry, and byte-identical Perplexity asset checks.
...
| 2. Tile-based overworld | Partially live: Archive A1 plus the complete Two Networks, Referral Vault, and Editor/Silent Read room pairs use real ground/wall/decoration Phaser tilemap layers from manifest-backed native 16x16 sheets; region/world scenes and gameplay-map scenes boot through direct links; older large overworld experiments remain quarantined under `experiments/overworld-wip/`. |
| 3. Collision | Live on `main`: player/action collision, gates, interactables, and room blockers use Arcade-style AABB patterns; Archive A1 and all six Network/Referral/Editor-Silent rooms derive perimeter collision from tile cells while preserving reachable locked-exit prompts. |
...
| 8. Room and dungeon scaffolding | Live on `main`: the Office and Guide teach staged carry -> route -> stamp -> gate loops; the stable-ID critical path is A1 -> N1 -> N2 -> R1 -> R2 -> E1 -> S1 -> DV1 -> G1; Archive A1 demonstrates the compact source puzzle -> room clear -> physical exit rhythm before the mandatory DANN-E Black Vault and Buckram bindery. |
...
- Two-room Editor's Labyrinth / Silent Read Tower sequence where StateChat produces a draft plan, a human editor adds the visible bracket, four evidence files route to human workstations, and three publication dockets earn the Red Pencil, Proof Lens, and Buckram Key without a quiz chain.
...
1. Redesign the legacy A2-D3 Archive annex as a deliberate optional backtracking loop with unique rewards; do not restore its superseded Clearance Token bypass.
