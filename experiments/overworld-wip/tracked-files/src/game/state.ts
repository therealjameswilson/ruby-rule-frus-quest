...
import { getTileRegistryReadout } from "../systems/TileRegistry";
import { getWorldQuestInventoryReadout } from "../systems/worldQuestInventory";
import type { FrusOverworldState } from "./world";
import type {
  ChoiceOption,
  DocumentCandidate,
...
interface GameState {
  ...
  overworld: FrusOverworldState | null;
...
export function setSceneState(sceneName: string, mode: GameMode, objective: string) {
  ...
  gameState.physicalVerification = null;
  gameState.roomTraversal = null;
  if (sceneName !== "WorldScene") gameState.overworld = null;
  gameState.finalGateCertification = null;
  refreshQuestWorkflowState();
}
...
export function setOverworldState(state: FrusOverworldState | null) {
  gameState.overworld = state
    ? {
        ...state,
        player: { ...state.player },
        ...
        discoveredRegionNames: [...state.discoveredRegionNames],
        viewport: { ...state.viewport },
        camera: state.camera ? cloneCameraReadout(state.camera) : null
      }
    : null;
  refreshQuestWorkflowState();
}

export function patchOverworldState(patch: Partial<FrusOverworldState>) {
  if (!gameState.overworld) return;
  gameState.overworld = {
    ...gameState.overworld,
    ...patch,
    player: patch.player ? { ...patch.player } : gameState.overworld.player,
    inventory: patch.inventory ? [...patch.inventory] : gameState.overworld.inventory,
    questFlags: patch.questFlags ? { ...patch.questFlags } : gameState.overworld.questFlags,
    visitedScreenIds: patch.visitedScreenIds ? [...patch.visitedScreenIds] : gameState.overworld.visitedScreenIds,
    discoveredRegionNames: patch.discoveredRegionNames ? [...patch.discoveredRegionNames] : gameState.overworld.discoveredRegionNames,
    viewport: patch.viewport ? { ...patch.viewport } : gameState.overworld.viewport,
    camera: patch.camera ? cloneCameraReadout(patch.camera) : gameState.overworld.camera
  };
  refreshQuestWorkflowState();
}

function cloneCameraReadout(camera: NonNullable<FrusOverworldState["camera"]>) {
  return {
    ...camera,
    logicalResolution: { ...camera.logicalResolution },
    canvasResolution: { ...camera.canvasResolution },
...
function currentAreaId(): AreaId {
  if (gameState.currentScene === "WorldScene" && gameState.overworld) return gameState.overworld.currentAreaId;
...
export function getRoomGraphReadout() {
  ...
  if (gameState.currentScene === "WorldScene" && gameState.overworld) {
    visitedRoomIds.add("O1");
    revealedRoomIds.add("O1");
  }
...
export function renderGameToText() {
  ...
  return JSON.stringify(
    {
      coordinateSystem: "origin top-left; x increases right; y increases down; logical canvas 256x240, overworld camera viewport 256x224 below fixed HUD",
      ...
      tileRegistry: getTileRegistryReadout(),
      reliability: gameState.reliability,
      ...
      overworldQuestItems: getWorldQuestInventoryReadout(gameState.inventory),
      workflowTools: getWorkflowToolReadout(),
      ...
      camera: gameState.overworld?.camera ?? null,
