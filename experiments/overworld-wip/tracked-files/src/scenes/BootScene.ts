...
import { registerUiKitFrames } from "../systems/artPackUi";
...
export class BootScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.json("interiors", "assets/data/interiors.json");
    this.load.json("worldScreens", "assets/data/worldScreens.json");
  ...
  create() {
    ...
    this.createTextures();
    this.registerArtPackAnimations();
    registerUiKitFrames(this.textures);
  ...
  private preloadArtPackAssets() {
    this.load.json("artPackManifest", ART_PACK_MANIFEST_PATH);
    for (const sheet of Object.values(ART_PACK_SHEETS)) {
      this.load.spritesheet(sheet.textureKey, sheet.path, {
        frameWidth: 256,
        frameHeight: 384
      });
    }
    for (const image of Object.values(ART_PACK_IMAGES)) {
      this.load.image(image.textureKey, image.path);
    }
    for (const tileset of Object.values(ART_PACK_TILESETS)) {
      this.load.image(tileset.textureKey, tileset.path);
    }
    this.load.spritesheet(ART_PACK_EXTRAS.portraits_cast.textureKey, ART_PACK_EXTRAS.portraits_cast.path, {
      frameWidth: ART_PACK_EXTRAS.portraits_cast.cellWidth,
      frameHeight: ART_PACK_EXTRAS.portraits_cast.cellHeight
    });
    this.load.spritesheet(ART_PACK_EXTRAS.items_collectibles.textureKey, ART_PACK_EXTRAS.items_collectibles.path, {
      frameWidth: ART_PACK_EXTRAS.items_collectibles.cellWidth,
      frameHeight: ART_PACK_EXTRAS.items_collectibles.cellHeight
    });
    this.load.spritesheet(ART_PACK_EXTRAS.effects_stamps.textureKey, ART_PACK_EXTRAS.effects_stamps.path, {
      frameWidth: ART_PACK_EXTRAS.effects_stamps.cellWidth,
      frameHeight: ART_PACK_EXTRAS.effects_stamps.cellHeight
    });
    this.load.spritesheet(ART_PACK_EXTRAS.stamps_text.textureKey, ART_PACK_EXTRAS.stamps_text.path, {
      frameWidth: ART_PACK_EXTRAS.stamps_text.cellWidth,
      frameHeight: ART_PACK_EXTRAS.stamps_text.cellHeight
    });
    this.load.image(ART_PACK_EXTRAS.ui_kit.textureKey, ART_PACK_EXTRAS.ui_kit.path);
  }
  ...
  private registerArtPackAnimations() {
    for (const [sheetKey, sheet] of Object.entries(ART_PACK_SHEETS)) {
      if (!this.textures.exists(sheet.textureKey)) continue;
      if (sheetKey === "sprite_statechat_terminal") {
        this.makeAnimation(sheet.textureKey, "idle", [0, 1, 2, 3], 3, -1);
        this.makeAnimation(sheet.textureKey, "blink", [4, 5, 6, 7], 5, -1);
        this.makeAnimation(sheet.textureKey, "stream", [8, 9, 10, 11], 6, -1);
        this.makeAnimation(sheet.textureKey, "off", [12], 1, 0);
        this.makeAnimation(sheet.textureKey, "query", [13], 1, 0);
        this.makeAnimation(sheet.textureKey, "success", [14], 1, 0);
        continue;
      }
      this.makeAnimation(sheet.textureKey, "idle-down", [frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "idle-down")], 1, 0);
      this.makeAnimation(sheet.textureKey, "idle-up", [frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "idle-up")], 1, 0);
      this.makeAnimation(sheet.textureKey, "idle-left", [frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "idle-left")], 1, 0);
      this.makeAnimation(sheet.textureKey, "idle-right", [frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "idle-right")], 1, 0);
      this.makeAnimation(sheet.textureKey, "walk-down", [
        frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "walk-down-0"),
        frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "walk-down-1")
      ], 5, -1);
      this.makeAnimation(sheet.textureKey, "walk-up", [
        frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "walk-up-0"),
        frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "walk-up-1")
      ], 5, -1);
      this.makeAnimation(sheet.textureKey, "walk-left", [
        frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "walk-left-0"),
        frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "walk-left-1")
      ], 5, -1);
      this.makeAnimation(sheet.textureKey, "walk-right", [
        frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "walk-right-0"),
        frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "walk-right-1")
      ], 5, -1);
      if (sheetKey === "sprite_dann_e") {
        this.makeAnimation(sheet.textureKey, "menace", [frameIndex("sprite_dann_e", "menace")], 1, 0);
        this.makeAnimation(sheet.textureKey, "attack", [frameIndex("sprite_dann_e", "attack")], 1, 0);
        this.makeAnimation(sheet.textureKey, "defeated", [frameIndex("sprite_dann_e", "defeated")], 1, 0);
      } else {
        this.makeAnimation(sheet.textureKey, "interact", [frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "interact")], 1, 0);
        this.makeAnimation(sheet.textureKey, "read", [frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "read")], 1, 0);
        this.makeAnimation(sheet.textureKey, "victory", [frameIndex(sheetKey as keyof typeof ART_PACK_SHEETS, "victory")], 1, 0);
      }
    }
  }

  private makeAnimation(textureKey: string, state: string, frames: number[], frameRate: number, repeat: number) {
    const key = `${textureKey}-${state}`;
    if (this.anims.exists(key) || frames.some((frame) => frame < 0)) return;
    this.anims.create({
      key,
