...
import { ART_PACK_SHEETS, HUMANOID_SCALE, frameIndex, rolePackSheetKey } from "../game/artPack";
...
export class CharacterCreateScene extends Phaser.Scene {
  ...
  private renderSelection() {
    ...
    const packSheetKey = rolePackSheetKey(role.id);
    if (packSheetKey && this.textures.exists(ART_PACK_SHEETS[packSheetKey].textureKey)) {
      this.sprite
        .setTexture(ART_PACK_SHEETS[packSheetKey].textureKey, frameIndex(packSheetKey, "idle-down"))
        .setScale(HUMANOID_SCALE)
        .setOrigin(0.5, 0.84);
    } else {
      this.sprite
        .setTexture(role.snesSpriteKey)
        .setScale(1)
        .setOrigin(0.5, 0.5);
    }
