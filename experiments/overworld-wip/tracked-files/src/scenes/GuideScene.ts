...
export class GuideScene extends Phaser.Scene {
  ...
  private drawAntagonistPlaque(x: number, y: number, label: string, accent: string) {
    ...
    if (label.includes("DANN-E") && this.textures.exists(ART_PACK_SHEETS.sprite_dann_e.textureKey)) {
      this.add
        .image(x, y + 11, ART_PACK_SHEETS.sprite_dann_e.textureKey, frameIndex("sprite_dann_e", "menace"))
        .setDepth(61)
        .setScale(HUMANOID_SCALE)
        .setOrigin(0.5, 0.84);
    }
    this.add.text(x, label.includes("DANN-E") ? y - 13 : y - 7, label, {
      fontFamily: "monospace",
      fontSize: label.includes("DANN-E") ? "5px" : "6px",
      color: accent,
      align: "center"
