...
try{
 for(const mobile of [false,true]){
  ...
  const stations=await p.evaluate(()=>window.game.scene.getScene('NetworkScene').children.list.filter(o=>o.name?.startsWith('classnet-station-')).map(c=>({name:c.name,depth:c.depth,y:c.y,desk:c.list.filter(o=>o.name==='classnet-research-desk').map(o=>({texture:o.texture.key,width:o.displayWidth})),labels:c.list.filter(o=>typeof o.text==='string').map(o=>o.text)})));
  assert.equal(stations.length,3);for(const s of stations){assert.equal(s.desk.length,1);assert(Math.abs(s.desk[0].width-58)<0.001);assert.equal(s.depth,s.y+12);}
