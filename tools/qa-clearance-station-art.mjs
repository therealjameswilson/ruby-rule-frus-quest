...
try{
 for(const mobile of [false,true]){
  ...
  const stations=await p.evaluate(()=>window.game.scene.getScene('NetworkScene').children.list.filter(o=>o.name?.startsWith('classnet-station-')).map(c=>({name:c.name,depth:c.depth,y:c.y,desk:c.list.filter(o=>o.name==='classnet-research-desk').map(o=>({texture:o.texture.key,width:o.displayWidth})),plaqueBottom:c.list.find(o=>o.name==="classnet-station-plaque").getBounds().bottom,labels:c.list.filter(o=>typeof o.text==='string').map(o=>o.text)})));
