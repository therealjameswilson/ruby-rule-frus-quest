...
try{for(const mobile of [false,true])for(const [name,stage,room]of [['editor','09-earned-referral-manifest','E1'],['proof','10-earned-editor','S1'],['production','11-earned-proof','S1']]){
 ...
 const info=await page.evaluate(()=>{const scene=window.game.scene.getScene('SilentReadScene');const desks=scene.children.list.filter(o=>o.name?.startsWith('proof-desk-'));const lurker=scene.danneLurker;const wall=scene.children.getByName('editorial-walls');const context=wall?.texture.getSourceImage().getContext('2d');const wallCells=[];for(let y=0;y<12;y++)for(let x=0;x<16;x++){const expected=(x===0||x===15||y===0||y===11)&&!((x===0||x===15)&&y>=4&&y<=6);wallCells.push({expected,opaque:context?.getImageData((x*16+8)*4,(y*16+8)*4,1,1).data[3]===255});}return {wallMaskMatches:wallCells.every(c=>c.expected===c.opaque),lurker:{key:lurker.sprite.texture.key,width:lurker.sprite.displayWidth,height:lurker.sprite.displayHeight,bodyWidth:lurker.bodyBounds().width,bodyHeight:lurker.bodyBounds().height},room:scene.currentRoomId,floor:scene.children.getByName('editorial-carpet-floor')?.texture.key,desks:desks.length,detailed:desks.filter(d=>d.list.some(o=>o.texture?.key==='research-props-v1')).length};});
 assert.equal(info.wallMaskMatches,true);assert.equal(info.lurker.key,'danne-boss-combat-hd');assert(Math.abs(info.lurker.width-23.04)<0.01);assert(Math.abs(info.lurker.height-34.56)<0.01);assert.equal(info.lurker.bodyWidth,18);assert.equal(info.lurker.bodyHeight,20);assert.equal(info.room,room);assert(info.floor);assert(info.desks>0);assert.equal(info.detailed,info.desks);assert.deepEqual(errors,[]);
