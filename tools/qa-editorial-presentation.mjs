...
try{for(const mobile of [false,true])for(const [name,stage,room]of [['editor','09-earned-referral-manifest','E1'],['proof','10-earned-editor','S1'],['production','11-earned-proof','S1']]){
 ...
 const info=await page.evaluate(()=>{const scene=window.game.scene.getScene('SilentReadScene');const desks=scene.children.list.filter(o=>o.name?.startsWith('proof-desk-'));const lurker=scene.danneLurker;return {lurker:{key:lurker.sprite.texture.key,width:lurker.sprite.displayWidth,height:lurker.sprite.displayHeight,bodyWidth:lurker.bodyBounds().width,bodyHeight:lurker.bodyBounds().height},room:scene.currentRoomId,floor:scene.children.getByName('editorial-carpet-floor')?.texture.key,desks:desks.length,detailed:desks.filter(d=>d.list.some(o=>o.texture?.key==='research-props-v1')).length};});
