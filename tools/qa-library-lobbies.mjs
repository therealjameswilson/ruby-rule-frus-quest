...
try{
 ...
 for(let i=0;i<assignments.length;i++){
  ...
  const info=await p.evaluate(()=>{const s=window.game.scene.getScene('PresidentialLibraryScene');const source=s.children.list.find(o=>o.text==='SOURCES');const a=s.player.sprite.getBounds(),b=source.getBounds();return {labels:s.marks.map(m=>{const b=m.getBounds();return {text:m.text,left:b.left,right:b.right,bottom:b.bottom};}),stacks:s.barriers.map(b=>({key:b.texture?.key,width:b.displayWidth,height:b.displayHeight,visible:b.visible})),id:s.assignment.library,desks:s.children.list.filter(o=>o.name.startsWith('library-desk-')&&o.texture.key==='research-props-v1').length,floor:!!s.children.getByName('editorial-carpet-floor'),walls:!!s.children.getByName('editorial-walls'),sourceClear:b.bottom<a.top||b.top>a.bottom||b.left>a.right||b.right<a.left};});
  assert.deepEqual(info.stacks.map(b=>[b.width,b.height,b.visible]),[[28,20,true],[54,8,true]]);assert(info.stacks.every(b=>b.key.startsWith('misfiled-folders-')));assert(info.labels.every(l=>l.left>=8&&l.right<=248&&l.bottom<160));assert(info.labels[3].text.includes('4 FILE PACKET'));assert.equal(info.id,id);assert.equal(info.desks,4);assert(info.floor&&info.walls&&info.sourceClear);assert.deepEqual(errors,[]);
