...
try {
  for (const mobile of [false, true]) {
    ...
    if (mobile) {
      ...
      const p = x => ({ x: b.x + x * b.width / 256, y: b.y + 178 * b.height / 240, id: 1 });
      await cdp.send('Input.dispatchTouchEvent', { type: 'touchStart', touchPoints: [p(40)] });
      await cdp.send('Input.dispatchTouchEvent', { type: 'touchMove', touchPoints: [p(66)] });
