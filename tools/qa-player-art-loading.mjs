...
const b=await engine.launch();const results=[];
...
try {
 ...
 for(const [appearance,stem] of roster){
  ...
  const mobile=process.env.FRUS_QA_PHONE_ONLY==='1'||appearance==='compiler_ada';
  const testFailure=appearance==='compiler_ada'||appearance==='compiler_clara';
  ...
  if(testFailure){await p.waitForFunction(()=>window.game?.scene.isActive('PlayerArtLoadScene')&&window.game.scene.getScene('PlayerArtLoadScene').failed);
   ...
   await p.screenshot({path:`${out}/phone-retry.png`});fail=false;if(mobile)await tap(128,159);else await p.keyboard.press('Enter');
  }
  ...
  assert.equal(pose.key,stem+'-attack-v1');assert.equal(pose.frames,12);assert(requests.every(r=>r===stem+'-attack-v1.png'));if(testFailure)assert(requests.length>=2);else assert.equal(requests.length,1);
  ...
  await p.screenshot({path:`${out}/${appearance}-attack.png`});assert.deepEqual(errors,[]);results.push({appearance,mobile,requests,pose,active,retry:testFailure,errors});await p.close();
 }
 ...
 await writeFile(`${out}/result.json`,JSON.stringify({engine:process.env.FRUS_QA_ENGINE??'chromium',freshTitleAttackRequests:0,totalBytesDeferredFromBoot:totalBytes,results},null,2));console.log(JSON.stringify({freshTitleAttackRequests:0,totalBytesDeferredFromBoot:totalBytes,roster:results.length,phoneRetry:true}));
