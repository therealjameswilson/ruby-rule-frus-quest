...
assert(await dock.isVisible());const room=await p.locator('canvas').first().boundingBox(),box=await dock.boundingBox();assert(box.y>=room.y+room.height+10);assert(box.y+box.height<=844-4);assert((await p.locator('[data-control=start]').boundingBox()).height>=44);
...
await p.goto('http://127.0.0.1:5211/');await p.waitForFunction(()=>window.game?.scene.isActive('TapToStartScene'));assert(!await dock.isVisible());results.titleHidden=true;assert.deepEqual(errors,[]);await writeFile(`${out}/result.json`,JSON.stringify({...results,errors,limitations:['Simulated Chromium touch; no physical iPhone','Player position fixture used for clear movement corridor']},null,2));console.log(results);
