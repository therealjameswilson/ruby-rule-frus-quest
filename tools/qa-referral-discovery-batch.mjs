...
try{
 ...
 await move(96,174);await move(232,174);await move(232,78);await move(128,78);await key('ArrowUp',800);
 ...
 assert.equal((await state()).sceneProgress.referralManifestCarried,1);
 assert.equal((await state()).sceneProgress.referralEquityPacketCarried,0);
 assert.equal((await state()).sceneProgress.referralDispatchBatchPrepared,1);
 assert(!(await state()).sceneProgress.referralEquityRouteComplete);
 assert(!(await state()).sceneProgress.referralManifestReviewComplete);
 const points=(await state()).documentPoints;
 await key();assert.equal((await state()).documentPoints,points);
 await page.reload();await page.waitForFunction(()=>JSON.parse(window.render_game_to_text()).scene==='TapToStartScene');await key('Enter');
 await page.waitForFunction(()=>JSON.parse(window.render_game_to_text()).scene==='ReferralVaultScene');await page.waitForTimeout(500);
 assert.equal((await state()).roomTraversal.currentRoomId,'R3');
 assert.equal((await state()).sceneProgress.referralManifestCarried,1);
 assert.equal((await state()).documentPoints,points);await shot('draft-reloaded');
 ...
 assert.deepEqual(errors,[]);console.log('PASS exploration-prepared batch, unapproved draft persistence, shelf shortcut and return');
